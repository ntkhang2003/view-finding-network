...
def create_database(tfr_file, image_folder, mtdb, offset, n, size, crops, n_crops):
    expr = re.compile(r".*/([0-9_a-f]*\.jpg)")

    print(f"Writing {len(crops)} crops of {n} images to {tfr_file}")
            ...
            for l in crops:
                try:
                    ...
                    example = tf.train.Example(features=tf.train.Features(feature={
                        ...
                        'image_raw': _bytes_feature(img_comb.tobytes()),
                        'img_file': _bytes_feature(match.group(1).encode('utf-8')),
                        'crop': _bytes_feature(np.array(crop).tobytes()),
                        'crop_type': _bytes_feature(info['crop_type'].encode('utf-8')),
                        'crop_scale': _float_feature(info['crop_scale'])
                    }))
                    writer.write(example.SerializeToString())
                except Exception as e:
                    print(f"Error processing image crop {l} of image {match.group(1)}: {e}")
                    continue
            if (k + 1) % 100 == 0:
                print(f"Wrote {k + 1} examples")
            k += 1
    return n
...
if __name__ == "__main__":
    ...
    if n_images < args.n_trn + args.n_val:
        print(f"Error: {n_images} images available, {args.n_trn + args.n_val} required for train/validation")
        exit()

    offset_val = create_database(args.training_db, args.image_folder, crop_db, 0, args.n_trn, cnn_input, range(args.n_crops), args.n_crops)
    val_images = create_database(args.validation_db, args.image_folder, crop_db, offset_val, args.n_val, cnn_input, range(args.n_crops), args.n_crops)
